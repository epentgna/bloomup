...
function Tracker({ account, onLogout }) {
  ...
  const [badApps, setBadApps] = useState(['Minecraft', 'Roblox', 'Steam', 'Discord', 'TikTok', 'Instagram', 'YouTube'])
  const [newBadApp, setNewBadApp] = useState('')
  const ignored = ['Electron', 'electron', 'Finder']
  ...
  useEffect(() => {
    setDoc(doc(db, 'users', account), { email: account, plantLevel, coins }, { merge: true }).catch(() => {})
  }, [plantLevel, coins])
  ...
  useEffect(() => {
    ...
    Promise.all(roster.map(async email => {
      try {
        const snap = await getDoc(doc(db, 'users', email))
        return snap.exists() ? { ...snap.data(), isMe: email === account } : { email, plantLevel: 1, coins: 0, isMe: email === account }
      } catch { return { email, plantLevel: 1, coins: 0, isMe: email === account } }
    })).then(entries => {
